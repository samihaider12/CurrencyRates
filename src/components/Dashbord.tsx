...
const Dashboard: React.FC = () => {
  ...
  const handleClearAllData = () => {
    if (window.confirm("Kya aap waqai saara data delete karna chahte hain? Ye wapas nahi aayega.")) {
...
const chartData = useMemo(() => {
  ...
  return (
    <Container sx={{ 
      ...
      {/* Date Filter Panel */}
<Box  sx={{ p: { xs: 2, md: 3 }, mb: 4, borderRadius: 2 }}>
  <Grid container spacing={2} alignItems="flex-end">
    <Grid size={{xs:12 ,sm:4,md:2}}>
      <Typography variant="caption" sx={{ fontWeight: 400, fontSize:"13px", display: 'block', mb: 0.5 }}>
        Start date
      </Typography>
      <TextField 
  ...
  {dateError && (
    <Typography color="error" sx={{ mt: 1, fontSize: '13px', fontWeight: 500 }}>
      ⚠️ {dateError}
    </Typography>
  )}
</Box>
<Grid container spacing={3} sx={{ mb: 4 }}>
  {[
    { label: 'Total Volume', value: `${stats.totalVolume.toFixed(2)}`, color: '#1976d2' },
    { label: 'Transactions', value: stats.totalTransactions, color: '#2e7d32' },
    { label: 'Top Currency', value: stats.mostUsed, color: '#ed6c02' }
  ].map((card, i) => (
    <Grid size={{ xs: 12, sm: 4 }} key={i}>
      <Paper sx={{ p: 2, textAlign: 'center', borderLeft: `5px solid ${card.color}`, borderRadius: 2 }}>
...
<CurrencyChart chartData={chartData} />
      ...
      <Box sx={{ borderBottom: 1, borderColor: 'divider', mb: 3, display: 'flex', gap: 4 }}>
        <Typography sx={{ pb: 1, borderBottom: '3px solid #1976d2', fontWeight: 'bold', cursor: 'pointer' }}>
          ALL ENTRIES
        </Typography>
      ...
      <Grid container spacing={3}>
        {filteredPairs.map((pair) => {
          ...
          return tableRows.length > 0 ? (
            <Grid size={{xs:12 , md:6}} key={`${pair.from}-${pair.to}`}>
              <ReusableTable
