...
const CurrencyRow = memo(({ code, symbol, onClick, open, isMobile, flagUrl }: any) => (
  <ListItem disablePadding sx={{ display: 'block' }}>
    <ListItemButton
      onClick={() => onClick(code)}
      sx={{ minHeight: "65px", px: 2, justifyContent: 'space-between' }}
    >
      <ListItemIcon sx={{ minWidth: 0, mr: (open || isMobile) ? 2 : 'auto' }}>
        <Avatar sx={{ 
          width: 35, height: 35, bgcolor: '#f5f5f5', 
          color: '#333', fontSize: '14px', fontWeight: 'bold',
      ...
      {(open || isMobile) && (
        <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', width: '100%' }}>
          <Typography sx={{ fontWeight: 500, color: '#555', fontSize: '14px' }}>{code}</Typography>
          <Avatar src={flagUrl} alt={code} sx={{ width: 24, height: 24, border: '1px solid #eee' }} />
        </Box>
      )}
    </ListItemButton>
    <Divider />
      ...
      <AppBar position="fixed" sx={{ zIndex: theme.zIndex.drawer + 1, bgcolor: '#1976d2' }}>
        <Toolbar sx={{ justifyContent: 'space-between' }}>
          ...
          <Box>
            ...
            {user ? (
              <Button sx={{ color: "red", textTransform: "none", bgcolor: 'rgba(255,255,255,0.1)' }} onClick={handleLogout}>Logout</Button>
            ) : (
